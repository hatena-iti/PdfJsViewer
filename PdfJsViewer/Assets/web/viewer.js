...
/*
  Additional modifications for PdfJsViewer project:
    ...
    - Adds free edit function. ("pencil" and "eraser" button)
*/
...
function consoleLog(message) {
    ...
    window.external.notify('{ "action":"log", "message":"[viewer][debug] ' + message + '" }');
};
function consoleWarn(message) {
    console.warn('[viewer] ' + message);
    window.external.notify('{ "action":"log", "message":"[viewer][warn] ' + message + '" }');
};
function consoleError(message) {
    console.error('[viewer] ' + message);
    window.external.notify('{ "action":"log", "message":"[viewer][error] ' + message + '" }');
};
...
function scrollIntoView(element, spot) {
  ...
  if (!parent) {
    ...
    consoleError('offsetParent is not set -- cannot scroll');
    return;
  }
...
var PDFFindController = {
  ...
  nextPageMatch: function() {
    if (this.resumePageIdx !== null)
      consoleError('There can only be one pending page.'); //console.error('There can only be one pending page.');
...
*/
    window.external.notify('{ "action":"save", "path":"' + url + '" }');
  },
  ...
  load: function pdfViewLoad(pdfDocument, scale) {
      ...
      if (PDFView.supportsPrinting) {
        pdfDocument.getJavaScript().then(function(javaScript) {
          if (javaScript.length) {
            ...
            consoleWarn('Warning: JavaScript is not supported');
            PDFView.fallback();
          }
    ...
    pdfDocument.getMetadata().then(function(data) {
      ...
      consoleLog('PDF ' + pdfDocument.fingerprint + ' [' +
                  info.PDFFormatVersion + ' ' + (info.Producer || '-') +
                  ' / ' + (info.Creator || '-') + ']' +
                  (PDFJS.version ? ' (PDF.js: ' + PDFJS.version + ')' : ''));
      ...
      if (info.IsAcroFormPresent) {
        ...
        consoleWarn('Warning: AcroForm/XFA is not supported');
        PDFView.fallback();
      }
    });
  },
  ...
  // Enables / Disables a free edit mode.
  setFreeEditMode: function pdfViewSetFreeEditMode(mode) {
    var originalFreeEditMode = this.freeEditMode;

    if ((originalFreeEditMode == 'pencil' && mode == 'pencil') ||
        (originalFreeEditMode == 'eraser' && mode == 'eraser')) {
      this.freeEditMode = 'none';

    } else {
      this.freeEditMode = mode;
    }

    var pencil = document.getElementById('pencil');
    var eraser = document.getElementById('eraser');

    if (this.freeEditMode == 'pencil') {
      pencil.className = 'toolbarButton pencil-selected';
      eraser.className = 'toolbarButton eraser';
      ...
      pencil.className = 'toolbarButton pencil';
      eraser.className = 'toolbarButton eraser-selected';
...
var PageView = function pageView(container, id, scale,
  ...
  this.draw = function pageviewDraw(callback) {
    ...
    if (this.renderingState !== RenderingStates.INITIAL) {
      ...
      consoleError('Must be in new state before drawing');
    }
    ...
    // Add a free edit canvas area
    var freeEditCanvas = document.createElement('canvas');
    freeEditCanvas.id = 'freeEdit' + this.id;
    freeEditCanvas.className = 'freeEdit';
    freeEditCanvas.width = canvas.width;
    freeEditCanvas.height = canvas.height;
    var context = freeEditCanvas.getContext('2d');
    context.strokeStyle = '#ff0000';
    context.lineWidth = 6;
    context.lineCap = 'round';
    div.appendChild(freeEditCanvas);

    // Add event listener to free edit canvas area
    ...
    freeEditCanvas.addEventListener(touchstart, function (event) {
      if (PDFView.freeEditMode != 'none') {
        PDFView.freeEditNowDrawing[this.id] = true;
    ...
    freeEditCanvas.addEventListener(touchmove, function (event) {
      if (PDFView.freeEditMode != 'none') {

        if (PDFView.freeEditNowDrawing[this.id]) {
          var context = this.getContext('2d');
    ...
    freeEditCanvas.addEventListener(touchend, function (event) {
      if (PDFView.freeEditMode != 'none') {
        PDFView.freeEditNowDrawing[this.id] = false;
  ...
  this.convertMatches = function textLayerBuilderConvertMatches(matches) {
    ...
    for (var m = 0; m < matches.length; m++) {
      ...
      if (i == bidiTexts.length) {
        ...
        consoleError('Could not find matching mapping');
      }
...
document.addEventListener('DOMContentLoaded', function webViewerLoad(evt) {
  ...
  document.getElementById('pencil').addEventListener('click',
    function () {
        PDFView.setFreeEditMode('pencil');
    });

  document.getElementById('eraser').addEventListener('click',
    function () {
        PDFView.setFreeEditMode('eraser');
    });
